document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateFilterCounts(flatTracks) {
    ...
    const counts = {
      ...
      top_music: flatTracks.filter((t) => t.type.toLowerCase() === "top_music")
        .length,
      top_all_times: flatTracks.filter(
        (t) => t.type.toLowerCase() === "top_all_times"
      ).length,
      trending: flatTracks.filter((t) => t.type.toLowerCase() === "trending")
        .length,
    };
    ...
    addButton.addEventListener("click", () => {
      Swal.fire({
        ...
        preConfirm: () => {
          ...
          const img = document.getElementById("profile-pic").files[0]
            ? URL.createObjectURL(
                document.getElementById("profile-pic").files[0]
              )
            : "";

          if (!nameMusic || !artist || !listType || !mp3) {
            Swal.showValidationMessage(
              "Tên bài hát, Nghệ sĩ, Loại danh sách và URL MP3 là bắt buộc."
            );
            return false;
          }
        ...
        if (result.isConfirmed) {
          const maxId =
            flattenTracks().length > 0
              ? Math.max(...flattenTracks().map((t) => t.id))
              : 25;
          const newTrack = {
          ...
          const category = tracksData.data.find(
            (cat) => cat[result.value.listType]
          );
          if (category) {
  ...
  function handleEdit(event) {
    ...
    Swal.fire({
              ...
              <div class="container" style="text-align: left; font-family: Arial, sans-serif;">
                  ...
                  <div class="row mb-2">
                      <div class="col-6">
                          ...
                          <div class="input-group">
                              ...
                              <input type="text" class="form-control" id="name-music" value="${
                                track.nameMusic
                              }" style="font-size: 12px; border-left: none; border-color: #ccc;" />
                          </div>
                      </div>
                      <div class="col-6">
                          <label class="form-label" style="color: #333; font-size: 12px;">Nghệ sĩ</label>
                          <div class="input-group">
                              ...
                              <input type="text" class="form-control" id="artist" value="${
                                track.artist
                              }" style="font-size: 12px; border-left: none; border-color: #ccc;" />
                          </div>
                      </div>
                  ...
                  <div class="row mb-2">
                      <div class="col-6">
                          <label class="form-label" style="color: #333; font-size: 12px;">Loại danh sách</label>
                          <select class="form-select" id="list-type" style="font-size: 12px; border-color: #ccc;">
                              <option value="Top_Music" ${
                                track.type === "Top_Music" ? "selected" : ""
                              }>Top Music</option>
                              <option value="Top_All_Times" ${
                                track.type === "Top_All_Times" ? "selected" : ""
                              }>Top All Times</option>
                              <option value="Trending" ${
                                track.type === "Trending" ? "selected" : ""
                              }>Trending</option>
                          </select>
                      </div>
                      <div class="col-6">
                          ...
                          <div class="input-group">
                              ...
                              <input type="url" class="form-control" id="mp3" value="${
                                track.mp3
                              }" style="font-size: 12px; border-left: none; border-color: #ccc;" />
                          </div>
                      </div>
      ...
      if (result.isConfirmed) {
        ...
        tracksData.data.find((cat) => cat[category])[category][index] =
          updatedTrack;
  ...
  if (searchInput) {
    ...
    console.error(
      "Không tìm thấy ô tìm kiếm (.form-control[placeholder='Tìm kiếm'])!"
    );
  }
